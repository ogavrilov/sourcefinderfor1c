...
Функция ПолучитьСписокТестов(Тестирование) Экспорт
	...
	ВсеТесты.Добавить("ТестДолжен_ПроверитьПолучениеИсходниковИзХранилищаКонфигурацииНужнойВерсии");
...
Процедура _ПроверитьПолучениеИсходниковИзХранилищаКонфигурации(НомерВерсииХранилища = Неопределено) Экспорт
	...
	Попытка
		...
		Если НомерВерсииХранилища <> Неопределено Тогда
			ОписаниеИсходника.Вставить("НомерВерсииХранилища", НомерВерсииХранилища);
		КонецЕсли;
		НастройкиВыполнения.Вставить("ОписаниеИсходника", ОписаниеИсходника);
	...
	// готовим образец результата
	ОбразецРезультата = Новый Массив;
	ОписаниеФайла = Новый Соответствие;
	ОписаниеФайла.Вставить("ВременныйКаталог", ВременныйКаталог + "\source_" + НастройкиВыполнения.Получить("ИдентификаторПроцесса"));
	ОписаниеФайла.Вставить("ИмяФайлаДоВременногоКаталога", "\src\Catalogs\ДополнительныеОтчетыИОбработки\Commands\ДобавитьВнешнююОбработкуОтчет\Ext\CommandModule.bsl");
	ОбразецРезультата.Добавить(ОписаниеФайла);
	Если НомерВерсииХранилища = Неопределено Тогда
		ОписаниеФайла = Новый Соответствие;
		ОписаниеФайла.Вставить("ВременныйКаталог", ВременныйКаталог + "\source_" + НастройкиВыполнения.Получить("ИдентификаторПроцесса"));
		ОписаниеФайла.Вставить("ИмяФайлаДоВременногоКаталога", "\src\Catalogs\ДополнительныеОтчетыИОбработки\Ext\ObjectModule.bsl");
		ОбразецРезультата.Добавить(ОписаниеФайла);
	КонецЕсли;
	ОписаниеФайла = Новый Соответствие;
	ОписаниеФайла.Вставить("ВременныйКаталог", ВременныйКаталог + "\source_" + НастройкиВыполнения.Получить("ИдентификаторПроцесса"));
	ОписаниеФайла.Вставить("ИмяФайлаДоВременногоКаталога", "\src\CommonModules\ОбщийМодуль1\Ext\Module.bsl");
	ОбразецРезультата.Добавить(ОписаниеФайла);
	ОписаниеФайла = Новый Соответствие;
	ОписаниеФайла.Вставить("ВременныйКаталог", ВременныйКаталог + "\source_" + НастройкиВыполнения.Получить("ИдентификаторПроцесса"));
	ОписаниеФайла.Вставить("ИмяФайлаДоВременногоКаталога", "\src\Ext\ManagedApplicationModule.bsl");
	ОбразецРезультата.Добавить(ОписаниеФайла);
...
Процедура ТестДолжен_ПроверитьПолучениеИсходниковИзХранилищаКонфигурацииНужнойВерсии() Экспорт
	_ПроверитьПолучениеИсходниковИзХранилищаКонфигурации(3);
КонецПроцедуры

Процедура ТестДолжен_ПроверитьПолучениеИсходниковИзФайлаКонфигурации() Экспорт
	// подготовка тестовых данных
	Попытка
		КаталогВременнойИБ = ВременныйКаталог + "\testbd";
		СоздатьКаталог(КаталогВременнойИБ);

		ФайлКонфигурации = ВременныйКаталог + "\testcf.cf";
		
		ЧтениеАрхива = Новый ЧтениеZipФайла(ТекущийСценарий().Каталог + "\fixtures\testcf.zip");
		...
		ОписаниеИсходника = Новый Соответствие;
		ОписаниеИсходника.Вставить("ТипИсходника", "cf");
		ОписаниеИсходника.Вставить("Source", ФайлКонфигурации);
		НастройкиВыполнения.Вставить("ОписаниеИсходника", ОписаниеИсходника);
...
Процедура ТестДолжен_ПроверитьПолучениеИсходниковИзФайлаРасширенияКонфигурации() Экспорт
	// подготовка тестовых данных
	Попытка
		КаталогВременнойИБ = ВременныйКаталог + "\testbd";
		СоздатьКаталог(КаталогВременнойИБ);

		ФайлКонфигурации = ВременныйКаталог + "\testcfe.cfe";
		
		ЧтениеАрхива = Новый ЧтениеZipФайла(ТекущийСценарий().Каталог + "\fixtures\testcfe.zip");
		ЧтениеАрхива.ИзвлечьВсе(ВременныйКаталог);
		ЧтениеАрхива.Закрыть();
		ЧтениеАрхива = Неопределено;
		...
		ОписаниеИсходника = Новый Соответствие;
		ОписаниеИсходника.Вставить("ТипИсходника", "cfe");
		ОписаниеИсходника.Вставить("Source", ФайлКонфигурации);
		НастройкиВыполнения.Вставить("ОписаниеИсходника", ОписаниеИсходника);
	...
	ОписаниеФайла = Новый Соответствие;
	ОписаниеФайла.Вставить("ВременныйКаталог", ВременныйКаталог + "\source_" + НастройкиВыполнения.Получить("ИдентификаторПроцесса"));
	ОписаниеФайла.Вставить("ИмяФайлаДоВременногоКаталога", "\src\CommonModules\ТВИПИ_ОбщийМодуль1\Ext\Module.bsl");
	ОбразецРезультата.Добавить(ОписаниеФайла);
	ОписаниеФайла = Новый Соответствие;
	ОписаниеФайла.Вставить("ВременныйКаталог", ВременныйКаталог + "\source_" + НастройкиВыполнения.Получить("ИдентификаторПроцесса"));
	ОписаниеФайла.Вставить("ИмяФайлаДоВременногоКаталога", "\src\DataProcessors\savehandlers\Forms\Форма\Ext\Form\Module.bsl");
	ОбразецРезультата.Добавить(ОписаниеФайла);
...
Процедура ТестДолжен_ПроверитьПолучениеИсходниковИзХранилищаРасширенияКонфигурации(НомерВерсииХранилища = Неопределено) Экспорт
	// подготовка тестовых данных
	Попытка
		КаталогВременнойИБ = ВременныйКаталог + "\testbd";
		СоздатьКаталог(КаталогВременнойИБ);

		КаталогХранилища = ВременныйКаталог + "\testcferepo";
		СоздатьКаталог(КаталогХранилища);
		
		ЧтениеАрхива = Новый ЧтениеZipФайла(ТекущийСценарий().Каталог + "\fixtures\testcferepo.zip");
		ЧтениеАрхива.ИзвлечьВсе(КаталогХранилища, РежимВосстановленияПутейФайловZIP.Восстанавливать);
		ЧтениеАрхива.Закрыть();
		ЧтениеАрхива = Неопределено;
		...
		ОписаниеИсходника = Новый Соответствие;
		ОписаниеИсходника.Вставить("ТипИсходника", "repocfe");
		ОписаниеИсходника.Вставить("Source", КаталогХранилища);
		ОписаниеИсходника.Вставить("User", "admin");
		ОписаниеИсходника.Вставить("Pwd", "admin");
		НастройкиВыполнения.Вставить("ОписаниеИсходника", ОписаниеИсходника);
	...
	ОписаниеФайла = Новый Соответствие;
	ОписаниеФайла.Вставить("ВременныйКаталог", ВременныйКаталог + "\source_" + НастройкиВыполнения.Получить("ИдентификаторПроцесса"));
	ОписаниеФайла.Вставить("ИмяФайлаДоВременногоКаталога", "\src\CommonModules\ТВИПИ_ОбщийМодуль2\Ext\Module.bsl");
	ОбразецРезультата.Добавить(ОписаниеФайла);
	ОписаниеФайла = Новый Соответствие;
	ОписаниеФайла.Вставить("ВременныйКаталог", ВременныйКаталог + "\source_" + НастройкиВыполнения.Получить("ИдентификаторПроцесса"));
	ОписаниеФайла.Вставить("ИмяФайлаДоВременногоКаталога", "\src\DataProcessors\savehandlers\Forms\Форма\Ext\Form\Module.bsl");
...
Процедура ТестДолжен_ПроверитьРаботуКомандойПоПутиКФайлу() Экспорт
	Попытка
		// подготовка тестовых данных
		КопироватьФайл(ТекущийСценарий().Каталог + "\fixtures\testfile1.bsl", ВременныйКаталог + "\testfile.bsl");
		...
		ФайлРезультата = ВременныйКаталог + "\result.json";
		НастройкиВыполнения.Вставить("ФайлРезультатаПроцесса", ФайлРезультата);
		
		// запишем настройки в файл
		ИмяФайлаНастроек = ВременныйКаталог + "\testoptions.json";
		...
		СтрокаЗапуска = "oscript """ + СтрЗаменить(ТекущийСценарий().Каталог, "tests", "") + "src\extractsource.os"" """ + ИмяФайлаНастроек + """ &>""" + ИмяФайлаВывода + """";
	...
	Результат = ОбщиеМетоды.ПрочитатьФайлJSON(ФайлРезультата);
