...
Процедура ПередЗапускомТеста() Экспорт
	...
	// для теста ТестДолжен_ПроверитьПолучениеИсходниковИзФайлаКонфигурации
	КопироватьФайл(ТекущийСценарий().Каталог + "\fixtures\extractsource_testconf.cf", ВременныйКаталог + "\extractsource_testconf.cf");
	// для теста ТестДолжен_ПроверитьПолучениеИсходниковИзФайлаРасширенияКонфигурации
	КопироватьФайл(ТекущийСценарий().Каталог + "\fixtures\extractsource_testconfe.cfe", ВременныйКаталог + "\testconfe.cfe");
	// для теста ТестДолжен_ПроверитьПолучениеИсходниковИзХранилищаКонфигурации
	СоздатьКаталог(ВременныйКаталог + "\testconfrepo");
	НайденныеФайлы = НайтиФайлы(ТекущийСценарий().Каталог + "\fixtures\extractsource_testconfrepo\", "*");
	Для Каждого НайденныйФайл Из НайденныеФайлы Цикл
		КопироватьФайл(НайденныйФайл.ПолноеИмя, ВременныйКаталог + "\testconfrepo\" + НайденныйФайл.Имя);
	КонецЦикла;
	// для теста ТестДолжен_ПроверитьПолучениеИсходниковИзХранилищаРасширенияКонфигурации
	СоздатьКаталог(ВременныйКаталог + "\testconfrepoe");
	НайденныеФайлы = НайтиФайлы(ТекущийСценарий().Каталог + "\fixtures\extractsource_testconfrepoe\", "*");
	Для Каждого НайденныйФайл Из НайденныеФайлы Цикл
		КопироватьФайл(НайденныйФайл.ПолноеИмя, ВременныйКаталог + "\testconfrepoe\" + НайденныйФайл.Имя);
	КонецЦикла;
