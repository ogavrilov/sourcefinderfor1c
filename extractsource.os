...
Функция ВыгрузитьОбработкиБазы(ВременныйКаталогИсходников)
	...
	ПараметрыКоманды.Добавить("/F""" + КаталогВременнойИБ + """");
	...
	Для Каждого ФайлОбработки Из ФайлыОбработок Цикл
		ВременныйКаталогИсходниковОбработок = СоздатьКаталогВременнойИБ(ВременныйКаталогИсходниковОбработок, ФайлОбработки.ИмяБезРасширения);
		Если ВременныйКаталогИсходниковОбработок <> Неопределено Тогда
			ФайлыДляОбработки.Вставить(ФайлОбработки.Имя, ВременныйКаталогИсходниковОбработок);
		КонецЕсли;
	КонецЦикла;
