...
Процедура ПоискВФайлахПоШаблону()
	...
	Для Каждого ОписаниеФайла Из КоллекцияФайловДляАнализа Цикл
		...
		//
		ЧтениеТекстовогоФайла = Новый ЧтениеТекста(ВременныйКаталог + "/" + ИмяФайлаДоВременногоКаталога);
		НомерСтроки = 1;
		ТекущийМетод = "";
		ТекущаяОбласть = "";
		...
		Пока ОчереднаяСтрока <> Неопределено Цикл
			...
			Если СтрНайти(ОчереднаяСтрокаНРег, "функция ") > 0 Тогда
				...
				ПозицияСкобки = СтрНайти(ОчереднаяСтрока, "(");
